...
Like email, fmsg is distributed - anyone can host a fmsg server for their domain with their users. Unlike email but like IM apps fmsg is efficient at conversational message exchanges. 
...
Messages are binary and verifiable by all peers. Messages are sent via a fmsg host to one or more recipients. Each message in a thread is linked to the previous using a cryptographic hash forming a directed acyclic graph. The wire encoding is defined here: [Message Definition](https://github.com/markmnl/fmsg/blob/docs/SPECIFICATION.md#definition), but an easier way to see the structure is as JSON:
