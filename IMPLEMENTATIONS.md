...
| [fmsg-mcp](https://github.com/markmnl/fmsg-mcp)            | [MCP](https://modelcontextprotocol.io) server giving any MCP-capable agent (Claude Code, Claude Desktop, Cursor, VS Code, claude.ai connectors) an fmsg address - send, reply, threads, reactions, attachments and wait-for-message over WebSocket; stdio or Streamable HTTP with per-client API keys |
| [fmsg-mcp-claude](https://github.com/markmnl/fmsg-mcp-claude) | MCP server for Claude Code and Claude Desktop that shares a Claude session as an fmsg thread and resumes any fmsg thread as Claude context |
