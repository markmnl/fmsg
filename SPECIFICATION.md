...
A recipient added in a batch who was not already in the original's _to_ is a participant of that add-to batch message only, not of the original message. A reply from such a recipient MUST therefore reference the add-to batch message via _pid_ — referencing the original message directly would fail the participant rule in [Notes on Message Definition](#notes-on-message-definition), because they are not a participant of the original. Replies from added recipients thus extend the batch's branch of the thread. An address appearing in both _to_ and _add to_ was already a participant of the original and may reply on either branch.
